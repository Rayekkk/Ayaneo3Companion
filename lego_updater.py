...
REPOSITORY = "Rayekkk/Ayaneo3Companion"
PLUGIN_NAME = "AYANEO 3 Companion"
ARCHIVE_ROOT = "Ayaneo3Companion"
...
class Updater:
    """Existing manual update API with verified, bounded downloads."""

    def __init__(self, *, releases_url: str, user_agent: str, log_prefix: str,
                 plugin_dir: str, asset_name_template: str, logger,
                 user_home: str = "", user_uid: int = -1, user_gid: int = -1):
        if releases_url != API_URL or asset_name_template != ARCHIVE_ROOT + "-{version}.zip":
            raise ValueError("Updater configuration must name the AYANEO 3 Companion repository.")
    ...
    def reset(self):
        """Reopen only after the owner has drained workers during plugin unload."""
        with self._lock:
            with self._publication_lock:
                self._closed.clear()
                self._cached = None
    ...
    def plugin_version(self):
        """Retain the existing public version helper used by hardware downloads."""
        return self._current_version()

    def ssl_context(self):
        return self._tls()

    def open_url(self, url, timeout=REQUEST_TIMEOUT, headers=None):
        metadata = url == API_URL
        if not metadata:
            parsed = urllib.parse.urlsplit(url)
            prefix = RELEASE_BASE + "/download/"
            if not url.startswith(prefix) or parsed.query:
                raise ValueError("The download does not belong to the Companion release repository.")
        self._ensure_open()
        ...
        response = opener.open(request, timeout=min(timeout, REQUEST_TIMEOUT))
        try:
    ...
    def _open(self, url, *, metadata):
        return self.open_url(url, timeout=REQUEST_TIMEOUT)
    ...
    def _parse_release(self, data, current):
        ...
        if data.get("html_url") != release_url:
            raise ValueError("The release does not belong to the Companion repository.")
        result = {
        ...
        if len(matching) != 1:
            if available:
                result["error"] = f"Release asset {name} is missing or is not unique."
            return result, None
        asset = matching[0]
        size = asset.get("size")
        ...
        if (asset.get("browser_download_url") != url or asset.get("state") != "uploaded" or
            ...
            return result, None
        result.update(asset_name=name, download_url=url, size=size, download_available=available)
    ...
    def _validate_archive(stream, version, cancel=None):
        ...
        with zipfile.ZipFile(stream) as archive:
            ...
            for filename in ("plugin.json", "package.json"):
                ...
                elif manifest.get("name") != "ayaneo3companion":
    ...
    def _save(self, release):
        ...
        try:
            ...
            candidate = f".companion-download-{secrets.token_hex(16)}.part"
    ...
    def download_latest(self) -> dict:
        """Download the latest release; installation remains the user's Decky action."""
        return self.download()

    def download(self, expected_version: str | None = None) -> dict:
        with self._lock:
            try:
                ...
                if expected_version is not None and result.get("latest_version") != expected_version:
