...
import lego_updater as updates
...
def package(version=NEW, extra=None, manifest=None):
    ...
    with zipfile.ZipFile(data, "w", zipfile.ZIP_DEFLATED) as archive:
        files = {
            ...
            "package.json": json.dumps({"name": "ayaneo3companion", "version": version}),
...
class _UpdateFixture(unittest.TestCase):
    def make_updater(self, **kwargs):
        ...
        return updates.Updater(plugin_dir=str(self.root), releases_url=updates.API_URL,
                               user_agent="Ayaneo3Companion", log_prefix="[test]", logger=logger,
                               asset_name_template="Ayaneo3Companion-{version}.zip", **kwargs)
...
class UpdateChecks(_UpdateFixture):
    ...
    def test_original_no_argument_download_latest_api_uses_verified_backend_metadata(self):
        with patch.object(self.updater, "_metadata", return_value=metadata()), \
                patch.object(self.updater, "_save", return_value="/home/user/Downloads/update.zip") as save:
            result = self.updater.download_latest()
        self.assertTrue(result["success"], result)
        self.assertEqual(result["version"], NEW)
        self.assertEqual(save.call_args.args[0]["sha256"], metadata()["assets"][0]["digest"][7:])

    def test_reset_after_unload_discards_old_metadata_and_reopens_network(self):
        with patch.object(self.updater, "_metadata", return_value=metadata()) as request:
            self.updater.check()
            self.updater.close()
            self.assertFalse(self.updater.check()["success"])
            self.updater.reset()
            self.assertTrue(self.updater.check()["success"])
        self.assertEqual(request.call_count, 2)

    def test_wrong_repository_configuration_is_rejected(self):
        with self.assertRaises(ValueError):
            updates.Updater(plugin_dir=str(self.root), releases_url="https://api.github.com/repos/other/project/releases/latest",
                            user_agent="test", log_prefix="[test]", logger=None,
                            asset_name_template="Ayaneo3Companion-{version}.zip")

    def test_untrusted_initial_urls_are_refused_before_any_request(self):
        for url in ("https://evil.test/archive.zip", "https://github.com/another/repo/archive.zip",
                    "http://github.com/Rayekkk/Ayaneo3Companion/releases/download/v1.0.3/file.zip"):
            with self.subTest(url=url), patch("lego_updater.urllib.request.build_opener") as build:
                with self.assertRaises(ValueError):
                    self.updater.open_url(url)
    ...
    def test_final_response_url_is_rechecked_and_closed_on_escape(self):
        ...
        with patch("lego_updater.urllib.request.build_opener") as build, \
...
class ArchiveChecks(unittest.TestCase):
    def validate(self, payload):
        updates.Updater._validate_archive(io.BytesIO(payload), NEW)
    ...
    def test_path_traversal_duplicate_windows_paths_and_file_directory_conflicts(self):
        for name in ("../evil", "/absolute", "Ayaneo3Companion/../evil", "Other/main.py",
                     "Ayaneo3Companion/C:evil",
                     "Ayaneo3Companion//evil", "Ayaneo3Companion/./evil",
                     "ayaneo3companion/main.py", "Ayaneo3Companion/MAIN.PY",
                     "Ayaneo3Companion/dist", "Ayaneo3Companion/control\nname"):
            with self.subTest(name=name), self.assertRaises(ValueError):
                self.validate(package(extra=[(name, "bad")]))

    def test_backslash_in_real_zip_headers_rejected_on_every_platform(self):
        payload = package(extra=[("Ayaneo3Companion/evil", "bad")])
        with self.assertRaises(ValueError):
            self.validate(payload.replace(b"Companion/evil", b"Companion\\evil"))
    ...
    def test_symlink_rejected(self):
        info = zipfile.ZipInfo("Ayaneo3Companion/link")
...
class DownloadFiles(_UpdateFixture):
    def setUp(self):
        ...
        self.updater = self.make_updater(user_home=str(self.home), user_uid=self.uid, user_gid=self.gid)
    ...
    def test_directory_changed_to_symlink_between_creation_check_and_open_is_rejected(self):
        ...
        with patch("lego_updater.os.open", side_effect=race_open):
            result = self.do_download()
        self.assertFalse(result["success"])
        self.assertEqual(list(outside.iterdir()), [])
    ...
    def test_exclusive_temporary_name_collision_preserves_the_preexisting_file(self):
        ...
        downloads.mkdir()
        os.chown(downloads, self.uid, self.gid)
        existing = downloads / ".companion-download-collision.part"
        existing.write_bytes(b"do not remove someone else's partial")
        with patch("lego_updater.secrets.token_hex", return_value="collision"):
            result = self.do_download()
        self.assertFalse(result["success"])
    ...
    def test_cancel_just_before_publication_retains_the_original_file(self):
        ...
        def stop_after_sync(fd):
            original_fsync(fd)
            self.updater.close()
        with patch("lego_updater.os.fsync", side_effect=stop_after_sync):
            result = self.do_download()
        self.assertFalse(result["success"])
